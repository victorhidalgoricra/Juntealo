-- Fix: overdue payment banner incorrectly replaces upcoming banner seconds after dashboard load.
--
-- Root cause: get_member_payment_notifications RPC does not return payout data.
-- The dashboard derives the "current turn" by counting completed payouts per junta.
-- It gets payouts from the Zustand store (fetchUserJuntaSnapshot), which uses a direct
-- table query subject to RLS. For regular members (rol=participante), the payouts
-- table RLS may return an empty array, causing currentCuota = 1 for all juntas.
-- With currentCuota = 1, the filter passes old vencida schedules (cuota_numero=1)
-- from past turns while filtering out the actual current pendiente schedule
-- (cuota_numero=2+), producing the wrong "overdue" banner.
--
-- Fix: include delivered payouts (entregado_en IS NOT NULL) in the RPC response
-- so the frontend can determine the current turn without depending on store payouts.
...
begin
  ...
  -- Build result with payouts included so frontend can compute current turn correctly
  return jsonb_build_object(
    'juntas', coalesce((
    ...
    -- Only delivered payouts (entregado_en IS NOT NULL) are needed for turn detection.
    -- Pre-assigned but undelivered payouts must NOT be counted as completed turns.
    'payouts', coalesce((
      select jsonb_agg(jsonb_build_object(
        'id',           po.id,
