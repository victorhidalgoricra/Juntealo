...
WITH junta_facts AS (
  ...
  SELECT 'registered' event_name,p.created_at occurred_at,p.id user_id,NULL::uuid junta_id FROM public.profiles p
  ...
  SELECT grain,date_trunc(grain,occurred_at)::date period_start,event_name,user_id,junta_id
  FROM events CROSS JOIN (VALUES ('week'::text),('month'::text)) g(grain)
), summary AS (
  SELECT grain,period_start,
    count(DISTINCT user_id) FILTER (WHERE event_name='registered') registered_users,
    count(DISTINCT user_id) FILTER (WHERE event_name='created_junta') users_who_created_junta,
    count(DISTINCT user_id) FILTER (WHERE event_name='joined_junta') users_who_joined_junta,
    count(DISTINCT junta_id) FILTER (WHERE event_name='created_junta') juntas_created,
    count(DISTINCT junta_id) FILTER (WHERE event_name='first_member') juntas_with_first_member,
    count(DISTINCT junta_id) FILTER (WHERE event_name='full') juntas_full,
    count(DISTINCT junta_id) FILTER (WHERE event_name='activated') juntas_activated,
    count(DISTINCT junta_id) FILTER (WHERE event_name='first_payment') juntas_with_first_confirmed_payment,
    count(DISTINCT junta_id) FILTER (WHERE event_name='first_cycle_completed') juntas_with_completed_cycle,
    count(DISTINCT junta_id) FILTER (WHERE event_name='finalized') juntas_finalized
  FROM periodized GROUP BY grain,period_start
)
SELECT *,round(100.0*users_who_joined_junta/nullif(registered_users,0),2) registration_to_participation_rate,
  round(100.0*juntas_with_first_member/nullif(juntas_created,0),2) created_to_first_member_rate,
  round(100.0*juntas_full/nullif(juntas_created,0),2) junta_fill_rate,
  round(100.0*juntas_activated/nullif(juntas_created,0),2) junta_activation_rate,
  round(100.0*juntas_with_first_confirmed_payment/nullif(juntas_activated,0),2) activation_to_first_payment_rate,
  round(100.0*juntas_finalized/nullif(juntas_created,0),2) completed_junta_rate
FROM summary ORDER BY grain,period_start;

-- ============================================================================
-- 10_retention_cohorts.sql
-- ============================================================================
WITH cohorts AS (
  SELECT id user_id,date_trunc('month',created_at)::date cohort_date FROM public.profiles
), activity AS (
  SELECT profile_id user_id,coalesce(validated_at,submitted_at,pagado_en) activity_at
  ...
  SELECT cohort_date,count(*) cohort_size FROM cohorts GROUP BY cohort_date
), retained AS (
  SELECT c.cohort_date,(extract(year FROM age(date_trunc('month',a.activity_at),c.cohort_date))*12
    + extract(month FROM age(date_trunc('month',a.activity_at),c.cohort_date)))::int period_number,
...
SELECT r.cohort_date,s.cohort_size,r.period_number,r.active_users,
  round(100.0*r.active_users/nullif(s.cohort_size,0),2) retention_rate
...
-- ============================================================================
-- 11_repeat_juntas.sql
-- ============================================================================
WITH participations AS (
  SELECT jm.profile_id user_id,jm.junta_id,jm.created_at joined_at,
    CASE WHEN j.estado::text='cerrada' THEN max(po.entregado_en) END completed_at_estimated
  FROM public.junta_members jm JOIN public.juntas j ON j.id=jm.junta_id
  LEFT JOIN public.payouts po ON po.junta_id=jm.junta_id
  GROUP BY jm.profile_id,jm.junta_id,jm.created_at,j.estado
), ordered AS (
  SELECT *,row_number() OVER(PARTITION BY user_id ORDER BY joined_at,junta_id) seq FROM participations
), per_user AS (
  SELECT user_id,min(joined_at) first_junta_date,min(completed_at_estimated) first_completed_junta_date,
    min(joined_at) FILTER(WHERE seq=2) second_junta_date,count(*) total_juntas_joined,
    count(*) FILTER(WHERE completed_at_estimated IS NOT NULL) total_juntas_completed
  FROM ordered GROUP BY user_id
), repeats AS (
  SELECT u.user_id,u.first_completed_junta_date,min(p.joined_at) next_junta_after_completion
  ...
  SELECT 'user'::text row_type,user_id,first_junta_date,first_completed_junta_date,second_junta_date,
    extract(epoch FROM (second_junta_date-first_junta_date))/86400.0 days_to_second_junta,
    total_juntas_joined,total_juntas_completed,NULL::bigint users_with_completed_junta,
    NULL::bigint users_who_joined_another_after_completion,NULL::numeric repeat_junta_rate,
    NULL::numeric average_days_to_next_junta,NULL::numeric median_days_to_next_junta FROM per_user
), summary AS (
...
-- ============================================================================
-- 12_payment_reliability.sql
-- ============================================================================
WITH obligations AS (
  SELECT ps.id schedule_id,ps.junta_id,ps.fecha_vencimiento due_date,payer.profile_id payer_id
  FROM public.payment_schedules ps JOIN public.junta_members payer ON payer.junta_id=ps.junta_id
   AND payer.orden_turno<>ps.cuota_numero
   AND payer.created_at<ps.fecha_vencimiento::timestamptz+interval '1 day'
   AND (payer.left_at IS NULL OR payer.left_at>=ps.fecha_vencimiento::timestamptz)
  ...
  SELECT date_trunc('month',o.due_date)::date month,
    CASE WHEN public.current_junta_score(o.payer_id)<=39 THEN '0-39'
      WHEN public.current_junta_score(o.payer_id)<=59 THEN '40-59'
      WHEN public.current_junta_score(o.payer_id)<=79 THEN '60-79' ELSE '80-100' END score_band,
    coalesce(l.payment_status='approved' OR l.estado::text='aprobado',false) confirmed,
    coalesce(l.submitted_at,l.pagado_en) paid_at,o.due_date::timestamptz+interval '1 day' deadline_at
  FROM obligations o LEFT JOIN latest l ON l.junta_id=o.junta_id
   AND l.schedule_id=o.schedule_id AND l.profile_id=o.payer_id
)
SELECT month,score_band,count(*) expected_payments,
  count(*) FILTER(WHERE confirmed) confirmed_payments,
  count(*) FILTER(WHERE confirmed AND paid_at<deadline_at) on_time_payments,
  count(*) FILTER(WHERE confirmed AND paid_at>=deadline_at) late_payments,
  count(*) FILTER(WHERE NOT confirmed) unpaid_payments,
  round(100.0*count(*) FILTER(WHERE confirmed AND paid_at<deadline_at)/nullif(count(*),0),2) on_time_payment_rate,
  round(100.0*count(*) FILTER(WHERE confirmed)/nullif(count(*),0),2) payment_completion_rate,
  round(avg(greatest(extract(epoch FROM (paid_at-deadline_at))/86400.0,0)) FILTER(WHERE confirmed),2) average_days_late,
  round(percentile_cont(.5) WITHIN GROUP(ORDER BY greatest(extract(epoch FROM
    (paid_at-deadline_at))/86400.0,0)) FILTER(WHERE confirmed)::numeric,2) median_days_late
FROM facts GROUP BY month,score_band ORDER BY month,score_band;

-- ============================================================================
-- 13_juntealo_volume.sql
-- Official amount: payments.submitted_amount, falling back to payments.monto.
-- ============================================================================
WITH confirmed AS (
  SELECT p.id,p.junta_id,p.profile_id payer_id,receiver.profile_id receiver_id,
    j.moneda currency,coalesce(p.submitted_amount,p.monto) confirmed_amount,
    coalesce(p.validated_at,p.submitted_at,p.pagado_en) confirmed_at
...
SELECT date_trunc('month',confirmed_at)::date month,currency,
  count(DISTINCT junta_id) active_juntas,count(DISTINCT payer_id) unique_payers,
  count(DISTINCT receiver_id) unique_receivers,count(DISTINCT id) confirmed_payments,
  sum(confirmed_amount) total_confirmed_volume,round(avg(confirmed_amount),2) average_payment,
  round(percentile_cont(.5) WITHIN GROUP(ORDER BY confirmed_amount)::numeric,2) median_payment,
  round(sum(confirmed_amount)/nullif(count(DISTINCT junta_id),0),2) average_volume_per_active_junta
FROM confirmed WHERE confirmed_at IS NOT NULL GROUP BY month,currency ORDER BY month,currency;
...
-- ============================================================================
WITH months AS (
  SELECT date_trunc('month',created_at)::date month FROM public.invitations
  UNION SELECT date_trunc('month',created_at)::date FROM public.referrals
), invites AS (
  SELECT date_trunc('month',created_at)::date month,count(*) invitations_sent,
    count(*) FILTER(WHERE estado::text='aceptada') invitations_accepted,
    count(DISTINCT enviado_por) inviters FROM public.invitations GROUP BY month
), referrals AS (
  SELECT date_trunc('month',created_at)::date month,count(*) registered,
    count(*) FILTER(WHERE status='active') activated FROM public.referrals GROUP BY month
)
SELECT m.month,coalesce(i.invitations_sent,0) invitations_sent,
  coalesce(i.invitations_accepted,0) invitations_accepted,
  coalesce(r.registered,0) users_registered_via_invitation,
  NULL::bigint users_joined_junta_via_invitation,
  round(i.invitations_sent::numeric/nullif(i.inviters,0),2) invites_per_inviter,
  round(100.0*i.invitations_accepted/nullif(i.invitations_sent,0),2) invite_acceptance_rate,
  NULL::numeric invite_registration_conversion,NULL::numeric invite_join_conversion,
  round(100.0*r.activated/nullif(r.registered,0),2) referral_first_payment_conversion,
  NULL::numeric approximate_k_factor
FROM months m LEFT JOIN invites i USING(month) LEFT JOIN referrals r USING(month)
ORDER BY m.month;

-- ============================================================================
-- 15_monthly_active_savers.sql
-- ============================================================================
WITH confirmed AS (
  SELECT p.id,p.junta_id,p.profile_id payer_id,receiver.profile_id receiver_id,
    j.moneda currency,coalesce(p.submitted_amount,p.monto) confirmed_amount,
  ...
  SELECT date_trunc('month',confirmed_at)::date month,currency,payer_id user_id FROM confirmed
  UNION SELECT date_trunc('month',confirmed_at)::date,currency,receiver_id FROM confirmed WHERE receiver_id IS NOT NULL
), monthly AS (
  SELECT date_trunc('month',confirmed_at)::date month,currency,
    count(DISTINCT payer_id) paying_users,count(DISTINCT receiver_id) receiving_users,
    count(DISTINCT junta_id) active_juntas,count(*) confirmed_payments,
    sum(confirmed_amount) total_confirmed_volume FROM confirmed
  WHERE confirmed_at IS NOT NULL GROUP BY month,currency
), mas AS (
  SELECT month,currency,count(DISTINCT user_id) monthly_active_savers FROM users GROUP BY month,currency
)
SELECT m.month,m.currency,mas.monthly_active_savers,m.paying_users,m.receiving_users,
  m.active_juntas,m.total_confirmed_volume,
  round(m.confirmed_payments::numeric/nullif(mas.monthly_active_savers,0),2) payments_per_active_saver,
  round(m.total_confirmed_volume/nullif(mas.monthly_active_savers,0),2) volume_per_active_saver
FROM monthly m JOIN mas USING(month,currency) ORDER BY m.month,m.currency;

-- ============================================================================
-- 16_product_metrics_summary.sql
...
WITH months AS (
  SELECT generate_series(date_trunc('month',least(
    ...
    date_trunc('month',current_date),interval '1 month')::date month
), currencies AS (
  SELECT DISTINCT moneda currency FROM public.juntas
), jf AS (
  SELECT j.id,j.moneda currency,j.created_at,j.participantes_max,
    count(*) FILTER(WHERE jm.estado::text<>'retirado') participants,
    CASE WHEN count(*) FILTER(WHERE jm.estado::text<>'retirado')>=j.participantes_max
      THEN max(jm.created_at) FILTER(WHERE jm.estado::text<>'retirado') END filled_at,
    min(ps.created_at) activated_at,
    CASE WHEN j.estado::text='cerrada' THEN max(po.entregado_en) END completed_at
  FROM public.juntas j LEFT JOIN public.junta_members jm ON jm.junta_id=j.id
  LEFT JOIN public.payment_schedules ps ON ps.junta_id=j.id
  LEFT JOIN public.payouts po ON po.junta_id=j.id GROUP BY j.id
), latest AS (
  SELECT DISTINCT ON(junta_id,schedule_id,profile_id) * FROM public.payments
  ORDER BY junta_id,schedule_id,profile_id,coalesce(submitted_at,pagado_en,created_at) DESC
), obligations AS (
  SELECT j.moneda currency,ps.junta_id,ps.id schedule_id,ps.fecha_vencimiento,
    payer.profile_id payer_id,receiver.profile_id receiver_id,
    coalesce(l.payment_status='approved' OR l.estado::text='aprobado',false) confirmed,
    coalesce(l.submitted_at,l.pagado_en) paid_at,coalesce(l.submitted_amount,l.monto) amount,
    coalesce(l.validated_at,l.submitted_at,l.pagado_en) confirmed_at
  FROM public.payment_schedules ps JOIN public.juntas j ON j.id=ps.junta_id
  ...
  SELECT date_trunc('month',confirmed_at)::date month,currency,payer_id user_id
    FROM obligations WHERE confirmed
  UNION SELECT date_trunc('month',confirmed_at)::date,currency,receiver_id
    FROM obligations WHERE confirmed AND receiver_id IS NOT NULL
), monthly AS (
  SELECT m.month,c.currency,
    (SELECT count(*) FROM public.profiles p WHERE date_trunc('month',p.created_at)::date=m.month) new_users,
    (SELECT count(DISTINCT e.profile_id) FROM public.user_activity_events e
      WHERE date_trunc('month',e.occurred_at)::date=m.month) active_users,
    (SELECT count(DISTINCT su.user_id) FROM saver_users su
      WHERE su.month=m.month AND su.currency=c.currency) monthly_active_savers,
    count(DISTINCT jf.id) FILTER(WHERE date_trunc('month',jf.created_at)::date=m.month) juntas_created,
    count(DISTINCT jf.id) FILTER(WHERE date_trunc('month',jf.activated_at)::date=m.month) juntas_activated,
    count(DISTINCT jf.id) FILTER(WHERE date_trunc('month',jf.completed_at)::date=m.month) juntas_completed,
    round(avg(jf.participants::numeric/nullif(jf.participantes_max,0)) FILTER
      (WHERE date_trunc('month',jf.created_at)::date=m.month),4) average_fill_rate,
    round(percentile_cont(.5) WITHIN GROUP(ORDER BY extract(epoch FROM (jf.filled_at-jf.created_at))/86400.0)
      FILTER(WHERE jf.filled_at IS NOT NULL AND date_trunc('month',jf.created_at)::date=m.month)::numeric,2) median_time_to_fill,
    round(percentile_cont(.5) WITHIN GROUP(ORDER BY extract(epoch FROM (jf.activated_at-jf.created_at))/86400.0)
      FILTER(WHERE jf.activated_at IS NOT NULL AND date_trunc('month',jf.created_at)::date=m.month)::numeric,2) median_time_to_activation,
    (SELECT count(*) FROM obligations o WHERE o.currency=c.currency AND o.confirmed
      AND date_trunc('month',o.confirmed_at)::date=m.month) confirmed_payments,
    (SELECT round(100.0*count(*) FILTER(WHERE o.confirmed)/nullif(count(*),0),2)
      FROM obligations o WHERE o.currency=c.currency AND o.fecha_vencimiento<=current_date
       AND date_trunc('month',o.fecha_vencimiento)::date=m.month) payment_completion_rate,
    (SELECT round(100.0*count(*) FILTER(WHERE o.confirmed AND o.paid_at<o.fecha_vencimiento::timestamptz+interval '1 day')
      ...
      WHERE o.currency=c.currency AND o.fecha_vencimiento<=current_date
       AND date_trunc('month',o.fecha_vencimiento)::date=m.month) on_time_payment_rate,
    (SELECT sum(o.amount) FROM obligations o WHERE o.currency=c.currency AND o.confirmed
      AND date_trunc('month',o.confirmed_at)::date=m.month) total_confirmed_volume,
    (SELECT round(100.0*count(*) FILTER(WHERE x.juntas_joined>=2)/nullif(count(*),0),2)
      FROM (SELECT jm.profile_id,count(DISTINCT jm.junta_id) juntas_joined
        FROM public.junta_members jm
        WHERE jm.created_at < m.month + interval '1 month'
        GROUP BY jm.profile_id) x) repeat_junta_rate,
    (SELECT count(*) FROM public.referrals r WHERE date_trunc('month',r.created_at)::date=m.month) referred_users,
    (SELECT round(100.0*count(*) FILTER(WHERE i.estado::text='aceptada')/nullif(count(*),0),2)
      FROM public.invitations i WHERE date_trunc('month',i.created_at)::date=m.month) invite_conversion_rate,
    (SELECT count(DISTINCT o.junta_id) FROM obligations o WHERE o.currency=c.currency AND o.confirmed
      AND date_trunc('month',o.confirmed_at)::date=m.month) active_juntas
  FROM months m CROSS JOIN currencies c LEFT JOIN jf ON jf.currency=c.currency
  GROUP BY m.month,c.currency
), calculated AS (
  SELECT *,round(100.0*juntas_activated/nullif(juntas_created,0),2) junta_activation_rate,
    round(100.0*referred_users/nullif(new_users,0),2) referral_percentage,
    round(100.0*(new_users-referred_users)/nullif(new_users,0),2) non_referral_percentage
  FROM monthly
)
SELECT *,
  round(100.0*(new_users-lag(new_users) OVER w)/nullif(lag(new_users) OVER w,0),2) user_growth_mom,
  round(100.0*(monthly_active_savers-lag(monthly_active_savers) OVER w)
    /nullif(lag(monthly_active_savers) OVER w,0),2) mas_growth_mom,
  round(100.0*(total_confirmed_volume-lag(total_confirmed_volume) OVER w)
    /nullif(lag(total_confirmed_volume) OVER w,0),2) volume_growth_mom,
  round(100.0*(active_juntas-lag(active_juntas) OVER w)
    /nullif(lag(active_juntas) OVER w,0),2) active_juntas_growth_mom
FROM calculated WINDOW w AS(PARTITION BY currency ORDER BY month)
