-- Every registered member belongs in the global ranking, including members
-- whose score is still zero. Previously, zero-score profiles were hidden for
-- everyone except the caller, which could make the leaderboard show one row.
