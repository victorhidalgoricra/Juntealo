-- Add maturity-window metrics without changing the cumulative cohort funnel from 090.
-- One aggregated RPC still serves the complete dashboard.
...
declare
  ...
  v_maturity_days constant integer := 7;
...
begin
  ...
  period_ranges as (
    select * from (values
      ('current'::text, v_start, v_now),
      ('previous'::text, v_previous_start, v_start)
    ) p(period_name, range_start, range_end)
  ),
  period_metrics as (
    select p.period_name,
      (select count(distinct s.user_id) from saver_facts s where s.occurred_at >= p.range_start and s.occurred_at < p.range_end) active_savers,
      (select count(distinct c.junta_id) from confirmed c where c.occurred_at >= p.range_start and c.occurred_at < p.range_end) juntas_with_movement,
      (select coalesce(sum(c.amount), 0) from confirmed c where c.currency='PEN' and c.occurred_at >= p.range_start and c.occurred_at < p.range_end) confirmed_volume,
      (select count(*) from creation_cohorts c where c.created_at >= p.range_start and c.created_at < p.range_end) juntas_created,
      (select count(*) from creation_cohorts c where c.created_at >= p.range_start and c.created_at < p.range_end
      ...
      (select count(*) from confirmed c where c.occurred_at >= p.range_start and c.occurred_at < p.range_end) confirmed_payments,
      (select count(*) from confirmed c where c.occurred_at >= p.range_start and c.occurred_at < p.range_end and c.is_on_time) on_time_payments
    from period_ranges p
  ),
  maturity_by_period as materialized (
    select p.period_name,
      count(*) filter(where j.created_at <= v_now-make_interval(days => v_maturity_days)) eligible,
      count(*) filter(where j.created_at <= v_now-make_interval(days => v_maturity_days)
        and j.first_filled_at is not null
        and j.first_filled_at <= j.created_at+make_interval(days => v_maturity_days)) filled_within_window,
      count(*) filter(where j.created_at <= v_now-make_interval(days => v_maturity_days)
        and j.activated_at is not null
        and j.activated_at <= j.created_at+make_interval(days => v_maturity_days)) activated_within_window
    from period_ranges p
    left join creation_cohorts c on c.created_at>=p.range_start and c.created_at<p.range_end
    left join public.juntas j on j.id=c.junta_id
    group by p.period_name
  ),
  current_cohort as materialized (
    select c.junta_id, c.created_at from creation_cohorts c
    where c.created_at >= v_start and c.created_at < v_now
  ),
  ...
  completed_participations as materialized (
    ...
    cross join boundaries b
    where e.event_type='junta_completed' and e.occurred_at>=b.previous_start and e.occurred_at<b.period_end
  ...
  repeat_eligible as materialized (
    ...
    from completed_participations where completed_at>=v_start and completed_at<v_now
    order by profile_id, completed_at, junta_id
  ),
  repeat_outcomes as (
  ...
  select jsonb_build_object(
    ...
    'kpis', jsonb_build_object(
      ...
      'activationWithin7d', jsonb_build_object(
        'current',(select jsonb_build_object('eligible',eligible,'achieved',activated_within_window,'rate',100.0*activated_within_window/nullif(eligible,0)) from maturity_by_period where period_name='current'),
        'previous',(select jsonb_build_object('eligible',eligible,'achieved',activated_within_window,'rate',100.0*activated_within_window/nullif(eligible,0)) from maturity_by_period where period_name='previous')),
    ...
    'health', jsonb_build_object(
      ...
      'repeatWithin30d',jsonb_build_object(
        'current',(select jsonb_build_object('eligibleUsers',eligible_users,'repeatedUsers',repeated_users,'rate',100.0*repeated_users/nullif(eligible_users,0)) from repeat_window_by_period where period_name='current'),
        'previous',(select jsonb_build_object('eligibleUsers',eligible_users,'repeatedUsers',repeated_users,'rate',100.0*repeated_users/nullif(eligible_users,0)) from repeat_window_by_period where period_name='previous')),
      ...
      'fillWithin7d',jsonb_build_object(
        'current',(select jsonb_build_object('eligible',eligible,'achieved',filled_within_window,'rate',100.0*filled_within_window/nullif(eligible,0)) from maturity_by_period where period_name='current'),
        'previous',(select jsonb_build_object('eligible',eligible,'achieved',filled_within_window,'rate',100.0*filled_within_window/nullif(eligible,0)) from maturity_by_period where period_name='previous')),
